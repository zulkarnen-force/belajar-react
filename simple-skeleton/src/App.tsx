...
export default  function App() {
  ...
  const [isFetchingData, setIFetchingData] =  useState(true);
  useEffect( () => {
    const fetchData = async () => {
      const res = await fetch('https://api.rawg.io/api/games?key='+import.meta.env.VITE_RAWG_API);
      const data = await res.json();
      console.log(data.results)
      await new Promise((resolve) => setTimeout(resolve, 2000))
      setGames(data.results)
      setIFetchingData(false)
    }
    fetchData()
  }, [])
  ...
  return (
        <div>
          ...
          { !isFetchingData &&
            ...
            {games.map((game) => (
              <div className="col-span-4 md:col-span-2 bg-slate-400 p-8 shadow" key={game.id}>
                <h1>{game.name}</h1>
